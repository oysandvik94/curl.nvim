...
M.execute_curl = function()
  ...
  if curl_alias ~= nil then
    curl_command.command = curl_command.command:gsub("^curl", curl_alias)
  end

  if curl_command.command == "" then
  ...
  if commands ~= nil and type(commands) == "table" then
    table.insert(commands, curl_command.command)
  else
    commands = curl_command.command
  end
